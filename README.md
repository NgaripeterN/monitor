...
5.  **Deploy (Two-Step Process):**
    1.  Deploy once **without** the `WEBHOOK_URL` variable.
...
*   **/setwallet `<12 or 24 word phrase>`**: Securely sets the wallet where you will receive payments. Use a new, empty wallet. **Your message is deleted immediately.**
*   **/addproduct `<Price>` `<Product Name>`**: Creates a product bundle and returns a `ProductID`.
*   **/addlink `<ProductID>` `<Link>`**: Adds a new link (e.g., for Dropbox, Telegram) to your product bundle.

...
*   **Seller Self-Service:** Sellers can register and manage their wallets, products, and links via secure Telegram commands.
...
5.  **Deploy (Two-Step Process):**
    ...
    2.  After it's live, copy the public URL, add it as the `WEBHOOK_URL` environment variable, and save. The bot will set its own webhook on the next startup.
...
*   **/addlink `<ProductID>` `<Link>`**: Adds a link (e.g., Dropbox, another Telegram group) to your product bundle.
...
*   Upon successful payment, the bot sends a message with all the links in the product bundle.
